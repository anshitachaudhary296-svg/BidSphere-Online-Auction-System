...
export const fetchLeaderboard = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${import.meta.env.VITE_BASE_URL}/api/v1/user/leaderboard`,
      {
        withCredentials: true,
      }
    );
...
export const forgotPassword = (email) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${import.meta.env.VITE_BASE_URL}/api/v1/user/password/forgot`,
      { email },
      {
        withCredentials: true,
        headers: { "Content-Type": "application/json" },
...
export const resetPassword = (token, data) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `${import.meta.env.VITE_BASE_URL}/api/v1/user/password/reset/${token}`,
      data,
      {
        withCredentials: true,
        headers: { "Content-Type": "application/json" },
