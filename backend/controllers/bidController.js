...
export const placeBid = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    // Capture previous highest bidder info before updating
    let previousBidderId = null;
    let previousBidAmount = auctionItem.currentBid;
    
    if (auctionItem.bids && auctionItem.bids.length > 0) {
      const sortedBids = [...auctionItem.bids].sort((a, b) => b.amount - a.amount);
      if (sortedBids[0].userId.toString() !== req.user._id.toString()) {
         previousBidderId = sortedBids[0].userId;
      }
    }
    ...
    // Emit Events
    notificationEmitter.emit("bidPlaced", {
    ...
    if (previousBidderId) {
      notificationEmitter.emit("outbid", {
        auctionId: auctionItem._id,
        previousBidderId,
        previousBidAmount,
        newBidAmount: amount
